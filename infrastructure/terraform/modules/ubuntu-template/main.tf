...
resource "null_resource" "create_cloudinit_config" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "  - |",
      "    cat > /etc/netplan/01-virtio.yaml << 'NETPLAN_EOF'",
      "    network:",
      "      version: 2",
      "      renderer: networkd",
      "      ethernets:",
      ...
      "qm set ${var.template_id} --cicustom 'user=local:snippets/template-${var.template_id}-config.yml' --ciuser ubuntu --cipassword ubuntu",
    ]
  }
}
