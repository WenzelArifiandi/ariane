# Commented out Proxmox-related outputs for now - focus on Cloudflare Access deployment only
...
output "cloudflare_access_application" {
  ...
  value = {
    id     = cloudflare_access_application.auth.id
    domain = cloudflare_access_application.auth.domain
    aud    = cloudflare_access_application.auth.aud
    name   = cloudflare_access_application.auth.name
  }
  sensitive = false
}
