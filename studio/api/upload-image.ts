...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const file: { buffer: Buffer; filename: string; mime: string } = await new Promise(
      (resolve, reject) => {
    ...
    // Detect HEIC/HEIF correctly (fix: escape the forward slash + dot)
    ...
    // Send a clear error message back to the browser to help us debug
