...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Find user by email
    const userResp = await fetch(`https://${domain}/api/v2/users-by-email?email=${encodeURIComponent(email)}`, {
    ...
    if (!user || !user.user_id) {
      ...
      res.status(202).json({ status: 'pending', message: 'Auth0 user not found yet; will apply after first login.' })
      return
    }

    // Merge app_metadata.approved = true
    const patchResp = await fetch(`https://${domain}/api/v2/users/${encodeURIComponent(user.user_id)}`, {
      method: 'PATCH',
      headers: {
