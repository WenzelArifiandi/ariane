...
function extractEmail(payload: unknown): string | null {
  ...
  if (typeof (data?.document as Record<string, unknown>)?.email === "string")
    return (data.document as Record<string, unknown>).email as string;
  if (data?._type === "accessRequest" && typeof data.email === "string")
    return data.email;
  if (
    data?.result &&
    typeof (data.result as Record<string, unknown>).email === "string"
  )
    return (data.result as Record<string, unknown>).email as string;
  return null;
}
