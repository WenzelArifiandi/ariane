...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!verified || !authenticationInfo) return new Response(JSON.stringify({ verified: false }), { status: 401 });
  ...
  const sessionCookie = makeCookie('session', sign(JSON.stringify(session), secret), { httpOnly: true, secure: isProd(), sameSite: 'Strict', path: '/', maxAge: 60 * 60 * 24 * 7 });
